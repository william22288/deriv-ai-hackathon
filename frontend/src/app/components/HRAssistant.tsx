...
import { Bot, Send, User, Sparkles, Clock, CheckCircle, ArrowRight } from "lucide-react";
...
export function HRAssistant() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      role: "assistant",
      content: "Hello! I'm your AI-powered HR assistant. I can help you with policy questions, update employee details, process requests, and route complex issues to the right team. How can I assist you today?",
  ...
  const employeeRequests: EmployeeRequest[] = [
    ...
    },
    {
      ...
      question: "How do I add my spouse to health insurance?",
      status: "new",
    ...
    },
    {
      ...
      type: "Payroll Issue",
      question: "My last paycheck shows incorrect tax withholding",
      status: "escalated",
    ...
    },
    {
      ...
      employee: "Sarah Lee",
      type: "Remote Work",
      question: "What's the policy for working from a different country?",
      status: "answered",
    ...
    },
    {
      ...
      employee: "David Kim",
      type: "Training",
      question: "When is the next compliance training session?",
      status: "new",
  ...
  const knowledgeBase = [
    ...
    {
      category: "Benefits",
      topics: ["Health insurance", "401(k)", "Life insurance", "Wellness programs"],
      queries: 189
    },
    {
      category: "Remote Work",
      topics: ["Work from home", "International work", "Equipment policy", "Expenses"],
      queries: 156
    },
    {
  ...
  const simulateAIResponse = async (userMessage: string): Promise<Message> => {
    ...
    if (lowerMessage.includes("pto") || lowerMessage.includes("vacation") || lowerMessage.includes("time off")) {
      ...
      actions = [
        { label: "Create Mobility Ticket", type: "escalate", status: "pending" as const }
      ];
    } else if (lowerMessage.includes("benefit") || lowerMessage.includes("insurance") || lowerMessage.includes("health")) {
      response = "I can help you with benefits enrollment:\n\n• Medical, dental, and vision coverage available\n• You can add dependents during open enrollment or within 30 days of a qualifying event\n• Life changes (marriage, birth) trigger special enrollment periods\n\nWould you like me to start a benefits update request or send you the enrollment forms?";
      actions = [
        { label: "Update Benefits", type: "action", status: "pending" as const },
        { label: "Send Enrollment Forms", type: "action", status: "pending" as const }
      ];
    } else if (lowerMessage.includes("payroll") || lowerMessage.includes("paycheck") || lowerMessage.includes("salary")) {
      response = "Payroll inquiries require verification for security. I'm escalating your request to the Payroll team with priority status. They typically respond within 4 business hours.\n\nTicket #PAY-2026-1847 has been created. You'll receive an email confirmation shortly.";
      actions = [
        { label: "Escalated to Payroll", type: "escalate", status: "completed" as const }
      ];
    } else if (lowerMessage.includes("update") && (lowerMessage.includes("address") || lowerMessage.includes("phone") || lowerMessage.includes("contact"))) {
      response = "I can help you update your contact information. For security purposes, I'll need to verify your identity first. Please check your email for a verification link.\n\nOnce verified, you can update:\n• Home address\n• Phone number\n• Emergency contact\n• Email preferences";
      actions = [
        { label: "Send Verification", type: "action", status: "completed" as const }
      ];
    } else {
      ...
      actions = [
        { label: "Connect with HR Specialist", type: "escalate", status: "pending" as const }
      ];
    }
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground">
          AI-powered chatbot for policy questions, request routing, and employee support
        </p>
      </div>
        ...
        <div className="space-y-6">
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className="text-sm font-medium">Answer Policy Questions</p>
                    <p className="text-xs text-muted-foreground">Instant responses from knowledge base</p>
                  </div>
                </div>
                <div className="flex items-start gap-2">
                  <CheckCircle className="h-4 w-4 text-green-600 mt-0.5" />
                  <div>
                    <p className="text-sm font-medium">Update Employee Details</p>
                    <p className="text-xs text-muted-foreground">Address, contact info, preferences</p>
                  </div>
                </div>
                <div className="flex items-start gap-2">
                  <CheckCircle className="h-4 w-4 text-green-600 mt-0.5" />
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Escalate complex issues to specialists</p>
                  </div>
                </div>
                <div className="flex items-start gap-2">
                  <CheckCircle className="h-4 w-4 text-green-600 mt-0.5" />
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Reduce HR team workload</p>
                  </div>
                </div>
              </div>
            </CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Employee inquiries being processed</CardDescription>
            </CardHeader>
            <CardContent>
              <Tabs defaultValue="all">
                ...
                <TabsContent value="all" className="space-y-2 mt-4">
                  {employeeRequests.slice(0, 3).map(request => (
                    <div key={request.id} className="p-2 rounded border space-y-1">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-medium">{request.employee}</span>
                        <Badge 
                          variant={
                            request.status === "answered" ? "default" :
                            request.status === "escalated" ? "destructive" :
                            "secondary"
                          }
                          className="text-xs"
                        >
                      ...
                      <p className="text-xs text-muted-foreground line-clamp-2">{request.question}</p>
                ...
                <TabsContent value="new" className="space-y-2 mt-4">
                  {employeeRequests.filter(r => r.status === "new").map(request => (
                    <div key={request.id} className="p-2 rounded border space-y-1">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-medium">{request.employee}</span>
                ...
                <TabsContent value="escalated" className="space-y-2 mt-4">
                  {employeeRequests.filter(r => r.status === "escalated").map(request => (
                    <div key={request.id} className="p-2 rounded border space-y-1">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-medium">{request.employee}</span>
                        <Badge variant="destructive" className="text-xs">{request.status}</Badge>
                      </div>
                      <p className="text-xs text-muted-foreground line-clamp-2">{request.question}</p>
                    </div>
                  ))}
